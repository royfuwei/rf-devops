...
# --- 4. 移除重複的 Certificate (如 rfjs-tls-secret) ---
if kubectl get certificate rfjs-tls-secret -n "$NAMESPACE" >/dev/null 2>&1; then
  echo "⚠️ Certificate 'rfjs-tls-secret' already exists. Deleting to prevent Helm install failure..."
  kubectl delete certificate rfjs-tls-secret -n "$NAMESPACE"
fi

# --- 5. 安裝或升級 Helm Release ---
VALUES_ARG=()
